...
User {
  ...
  role: enum [admin, editor]  // 未登入訪客 = reader（無 user row）
}
```

- Stage 1 只有兩個 role：admin（可發布）和 editor（可編輯、校稿）
- 未來 Stage 2+ 會擴充為完整的 permission system，加入 Reader 實體、BookClub、Sharing 等社群功能
- 設計哲學：每個人面對神話語時，都是謙卑的讀者（Reader 是基本姿態）；editor/admin 是疊加在 user 上的服事角色，不取代 reader 身份

### 內容架構：Publication → Article → Block

```
Publication {
  id, type: enum [magazine, book], title, description
  // magazine = 按日期排序（BOA 是第一本 magazine）
  // book = 按章節排序（未來擴充）
}
...
Template {
  ...
  blockDefinitions: BlockDefinition[]  // 定義這個 template 有哪些 blocks、順序、type
}
```
...
### BOA Standard Template（97% 的文章格式）
...
1. 先查 DB 快取
2. 快取沒有才打外部 API（目前使用信望愛 API，未來可替換）
...
```
寫作同工 (editor)
  → 建立/編輯 Article（status: draft）

校稿同工 (editor)
  → 標記為已校閱（status: reviewed）

傳道人 (admin)
  → 按下發布（status: published）
```
...
- [ ] 每日靈修內容頁（public 瀏覽）
- [ ] 歷史內容列表頁（先做 list view）
- [ ] 編輯表單（Template + typed blocks）
- [ ] 圖片批次上傳（一次上傳一週的背誦金句圖片）
- [ ] 校稿 + 發布簽核流程
- [ ] 簡單登入（admin / editor 兩個 role）
- [ ] LINE 推播（簡單版）
- [ ] PDF 匯出（簡單版）
- [ ] 舊內容 Migration（先搬最近 2-3 個月，建立 SOP）
